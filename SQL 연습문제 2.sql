...
insert into `bank_account` values ('101-23-1007','L3','주택담보대출','750210-1000002',2700000,'2002-09-23');
...
group by `t_a_no` having `합계` >= 100000 order by `합계` desc;
...
#실습2-26
select a.a_no as '계좌번호', a.a_item_name as '계좌이름', b.c_no as '주민번호', b.c_name as '고객명', a.a_balance as '현재잔액' 
...
#실습2-28
select `t_no` as '거래번호', `t_a_no` as '계좌번호', `a_c_no` as '고객번호(주민번호)',
`t_dist` as '거래구분', `t_amount` as '거래금액', `t_datetime` as '거래일자' 
from `bank_account` as a
...
group by `c_no` order by `t_dist`, `거래건수` desc;